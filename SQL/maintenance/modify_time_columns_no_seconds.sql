...
PRINT N'';
PRINT N'=== 驗證資料完整性 ===';
PRINT N'總記錄數: ' + CAST((SELECT COUNT(*) FROM student_schedules) AS NVARCHAR(10));
PRINT N'有效記錄數: ' + CAST((SELECT COUNT(*) FROM student_schedules WHERE is_active = 1) AS NVARCHAR(10));

-- 檢查時間格式
SELECT 
    '時間格式檢查' as 檢查項目,
    COUNT(*) as 總記錄數,
    SUM(CASE WHEN start_time LIKE '[0-9][0-9]:[0-5][0-9]' OR start_time LIKE '[0-9]:[0-5][0-9]' THEN 1 ELSE 0 END) as 正確格式數
