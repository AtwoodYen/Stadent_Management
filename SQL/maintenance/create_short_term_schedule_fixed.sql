...
-- 4. 建立查詢視圖
IF EXISTS (SELECT * FROM sys.views WHERE name = 'view_short_term_schedules_summary')
    DROP VIEW [dbo].[view_short_term_schedules_summary];

CREATE VIEW [dbo].[view_short_term_schedules_summary] AS
SELECT 
    sts.[id],
    sts.[student_id],
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_get_week_dates]') AND type in (N'FN', N'IF', N'TF'))
    DROP FUNCTION [dbo].[fn_get_week_dates];

CREATE FUNCTION [dbo].[fn_get_week_dates] (@input_date DATE)
RETURNS TABLE
AS
RETURN
(
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_get_available_time_slots]') AND type in (N'FN', N'IF', N'TF'))
    DROP FUNCTION [dbo].[fn_get_available_time_slots];

CREATE FUNCTION [dbo].[fn_get_available_time_slots]()
RETURNS TABLE
AS
RETURN
(
