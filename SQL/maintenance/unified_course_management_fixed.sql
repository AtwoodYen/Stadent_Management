...
ELSE
BEGIN
    PRINT N'courses_categories 資料表已存在';
END
GO
...
ELSE
BEGIN
    PRINT N'courses 表的 category_id 欄位已存在';
END
GO
...
ELSE
BEGIN
    PRINT N'teacher_courses 表的 category_id 欄位已存在';
END
GO
...
ELSE
BEGIN
    PRINT N'teacher_courses 表的外鍵約束已存在';
END
GO
...
PRINT N'6. 新增課程分類方式：';
PRINT N'   - 在 courses_categories 表中新增記錄';
PRINT N'   - 為師資在 teacher_courses 表中新增對應的教學能力';
PRINT N'   - 在 courses 表中新增對應的課程';
GO 