...
PRINT N'1. 檢查重複的分類名稱（去除前後空格）：';
SELECT 
    LTRIM(RTRIM(category_name)) as clean_name,
    COUNT(*) as count,
    STRING_AGG(CAST(id as NVARCHAR(10)) + '(' + category_code + ')', ', ') as ids
FROM courses_categories
GROUP BY LTRIM(RTRIM(category_name))
HAVING COUNT(*) > 1
ORDER BY LTRIM(RTRIM(category_name));
GO
...
SELECT 
    ...
    LTRIM(RTRIM(cc1.category_name)) as category_name
FROM courses_categories cc1
INNER JOIN courses_categories cc2 ON LTRIM(RTRIM(cc1.category_name)) = LTRIM(RTRIM(cc2.category_name))
...
-- 如果沒有找到對應關係，手動建立
IF NOT EXISTS (SELECT 1 FROM #category_mapping)
BEGIN
    PRINT N'手動建立對應關係...';
    
    -- Python
    INSERT INTO #category_mapping (source_id, source_code, target_id, target_code, category_name)
    SELECT 25, '_PYTHON', 1, 'PYTHON', 'Python';
    
    -- Swift
    INSERT INTO #category_mapping (source_id, source_code, target_id, target_code, category_name)
    SELECT 26, '_SWIFT', 34, 'SWIFT', 'Swift';
    
    -- Unity
    INSERT INTO #category_mapping (source_id, source_code, target_id, target_code, category_name)
    SELECT 28, '_UNITY', 35, 'UNITY', 'Unity';
    
    -- 遊戲開發
    INSERT INTO #category_mapping (source_id, source_code, target_id, target_code, category_name)
    SELECT 29, '_遊戲開發', 9, 'GAME_DEV', '遊戲開發';
    
    -- 機器學習
    INSERT INTO #category_mapping (source_id, source_code, target_id, target_code, category_name)
    SELECT 30, '_機器學習', 7, 'MACHINE_LEARNING', '機器學習';
